...
class KuudraSplits {
    ...
    fun onChat(event: ClientChatReceivedEvent) {
        ...
        when (message) {
            ...
            "[NPC] Elle: POW! SURELY THAT'S IT! I don't think he has any more in him!" -> {
                Minecraft.getMinecraft().thePlayer.addChatMessage(ChatComponentText(MeowMod.MEOW_PREFIX + EnumChatFormatting.GOLD + " Stun Kill" + EnumChatFormatting.WHITE + "took ${splits[currentSplit] / 20}s "))
                currentSplit++
            }
            ...
            "[NPC] Elle: Good job everyone. A hard fought battle come to an end. Let's get out of here before we run into any more trouble!" -> {
                ...
                Minecraft.getMinecraft().thePlayer.addChatMessage(ChatComponentText(MeowMod.MEOW_PREFIX + EnumChatFormatting.GRAY + " Fuel" + EnumChatFormatting.WHITE + " took ${splits[2] / 20}s"))
                Minecraft.getMinecraft().thePlayer.addChatMessage(ChatComponentText(MeowMod.MEOW_PREFIX + EnumChatFormatting.GOLD + " Stun Kill" + EnumChatFormatting.WHITE + " took ${splits[3] / 20}s"))
                Minecraft.getMinecraft().thePlayer.addChatMessage(ChatComponentText(MeowMod.MEOW_PREFIX + EnumChatFormatting.GRAY + " Fuel" + EnumChatFormatting.WHITE + " and" + EnumChatFormatting.GOLD + " Stun Kill" + EnumChatFormatting.WHITE + " took ${splits[2] / 20 + splits[3] / 20}s"))
                Minecraft.getMinecraft().thePlayer.addChatMessage(ChatComponentText(MeowMod.MEOW_PREFIX + EnumChatFormatting.DARK_RED + " Kuudra Down" + EnumChatFormatting.WHITE + " took ${splits[4] / 20}s"))
                for (i in splits.indices) splits[i] = 0
            }

            else -> {
                if (message.endsWith(" recovered a Fuel Cell and charged the Ballista! (100%)")) {
                    Minecraft.getMinecraft().thePlayer.addChatMessage(ChatComponentText(MeowMod.MEOW_PREFIX + EnumChatFormatting.GRAY + " Fuel" + EnumChatFormatting.WHITE + " took ${splits[currentSplit] / 20}s "))
                    currentSplit++
                }
    ...
    fun onTick(event: ClientTickEvent) {
        ...
        if (currentSplit in 0 until 5) splits[currentSplit]++
    }
}
